...
- add [`CircuitPEPOSimpleUpdate`](quimb.tensor.circuit.CircuitPEPOSimpleUpdate): a Heisenberg picture quantum circuit simulator that evolves an observable (a PEPO on an arbitrary geometry of ``edges``) backwards through the circuit with 'simple update' style gauging, then contracts it with the initial state to give ``local_expectation`` values like {math}`\langle 0 | U^\dagger G U | 0 \rangle`. Gates are stored lazily and only applied when an expectation is requested, gates outside the reverse lightcone of the observable are skipped, and the evolved operator can also be returned on its own with ``evolve_operator`` or with the initial state sandwiched in with ``sandwich_operator``.
