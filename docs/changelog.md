...
- add [`CircuitPEPOSimpleUpdate`](quimb.tensor.circuit.CircuitPEPOSimpleUpdate): a Heisenberg picture quantum circuit simulator. Gates are recorded lazily; on `local_expectation` the observable is built as a PEPO and evolved *backwards* through the gates (each applied as a simple-update compressed sandwich), before being contracted with the initial all-zeros state to give :math:`\langle 0 | U^\dagger G U | 0 \rangle`. Gates outside the observable's reverse lightcone are skipped, making local expectations of shallow circuits cheap even on large lattices.
