...
- Automatic generalized loop size (``max_size=None`` in [`tn.gen_gloops`](#TensorNetwork.gen_gloops), or ``gloops=None`` in loop expansions): grow loops until they cover every target tid or site. Previously, loop growth stopped at the first and smallest loop. On non-uniform geometries, this could leave some sites without a loop. Targets not in a loop are ignored. A ``UserWarning`` is raised if these targets were explicit or if the network is a tree and has no loops. Loops larger than the resolved size are no longer returned. Use ``max_size="min"`` or ``gloops="min"`` for the old behavior.
...
- [`TensorNetwork.gen_gloops`](#TensorNetwork.gen_gloops): fix automatic size selection and joins when target tensors may dangle. Add a separate ``allow_dangling`` option, on by default for local expectations and partial traces. ``"alldangle"`` and ``"anydangle"`` remain as aliases.
