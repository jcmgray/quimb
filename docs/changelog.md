...
- TN 1D compression: add 'successive deterministic compression' (``method="sdc"``) and its ``sdc-oversample`` variant, based on https://arxiv.org/abs/2601.19650.
- [`MatrixProductOperator.gate_sandwich_with_auto_swap`](#MatrixProductOperator.gate_sandwich_with_auto_swap): apply a two-site gate sandwich while maintaining the MPO canonical form, tracking its orthogonality center, optionally stripping the center-tensor exponent, and automatically swapping long-range sites together and back.
- 1D compression: generate random tensors for ``src``, ``srcmps``, and ``fit`` methods with autoray's (v0.10.0+) ``random.array``, which matches device and dtype. These methods and their oversampling variants also accept a ``seed`` or explicit random generator. The default uses the array backend's global random state.
- 1D compression: ``fit`` with ``bsz=1`` now runs for fermionic tensor networks, rather than raising ``NotImplementedError`` for any of them. It warns instead when the network contains tensors of odd parity, where the result is likely incorrect.
- gating: add a ``dagger`` option to [`tensor_network_gate_inds`](#tensor_network_gate_inds) (`tn.gate_inds`), [`tensor_network_gate_sandwich_inds`](#tensor_network_gate_sandwich_inds) (`tn.gate_sandwich_inds`), [`tensor_network_ag_gate`](#tensor_network_ag_gate) (thus `tn.gate`, `tn.gate_sandwich`, `tn.gate_upper`, `tn.gate_lower`), [`tensor_network_ag_gate_simple`](#tensor_network_ag_gate_simple) (`tn.gate_simple`, including the long range variant) and [`TensorNetworkGenOperator.gate_sandwich_with_op_lazy`](#TensorNetworkGenOperator.gate_sandwich_with_op_lazy), which applies $G^\dagger$ in place of $G$, e.g. turning the sandwich $G A G^\dagger$ into $G^\dagger A G$, as required for Heisenberg evolution. This saves conjugate transposing the gate by hand, which for a gate supplied as a tensor rather than a matrix also means reshaping it first.
- gating: add a matching ``transpose`` option to the functions above, applying $G^T$ in place of $G$ with no conjugation, and to [`MatrixProductState.gate_nonlocal`](#MatrixProductState.gate_nonlocal), which forwards it to [`gate_with_submpo`](#MatrixProductState.gate_with_submpo).
- [`Tensor.gate`](#Tensor.gate): the ``transposed`` argument is renamed ``transpose``, matching ``dagger`` and the ``transpose`` of the operator-network gating methods. The old name still works but raises a ``FutureWarning``.
...
- add [`LocalHamGen.get_trotter_gates`](#LocalHamGen.get_trotter_gates), which returns the sequence of local gates for a first, second or fourth order Trotter approximation to $\exp(x H)$, over any number of steps. The terms are grouped into commuting layers. Each [`TrotterGate`](#TrotterGate) unpacks as the pair ``U, where``, so ``for U, where in gates: psi.gate_(U, where)`` works, and has the attributes ``frac``, ``layer`` and ``step``. Consecutive applications of the same layer are fused by default. The product formula on its own is available as [`trotter_schedule`](#trotter_schedule). Inherited by [`LocalHam1D`](#LocalHam1D), [`LocalHam2D`](#LocalHam2D) and [`LocalHam3D`](#LocalHam3D).
- [`build_mpo_propagator_trotterized`](#LocalHam1D.build_mpo_propagator_trotterized) and [`build_pepo_propagator_trotterized`](#LocalHam2D.build_pepo_propagator_trotterized): add an ``order`` option, defaulting to 1 as before. The 1D version also gains the ``ordering`` option its 2D counterpart already had. Both now build the gate sequence with [`get_trotter_gates`](#LocalHamGen.get_trotter_gates).
- belief propagation: all BP classes now take ``diis`` at `__init__` as well as  ``run``, alongside ``damping``. If given to [`run`](#BeliefPropagationCommon.run), they update the defaults.
...
- [`safe_inverse`](#safe_inverse): take a scalar maximum when given a single vector, rather than always reducing over the last axis and broadcasting back. This fixes ``mode="projector"`` boundary contraction for block sparse backends such as `symmray`, which raised ``TypeError``.
...
- [`D2BP.compress`](#D2BP.compress) and [`D2BP.gauge_symmetric`](#D2BP.gauge_symmetric): preserve positive messages and full-rank identities for fermionic tensor networks.
- [`TensorNetwork.conj`](#TensorNetwork.conj): use ``output_inds`` to select the global output legs for fermionic conjugation phases, including for subnetworks.
- [`Tensor.conj`](#Tensor.conj): add matching ``output_inds`` control for a single tensor.
- SRC compression: fix bug where svd rather than qr was used by default to orthogonalize the sketched columns
- [`TEBD`](#TEBD): ``order=4`` was only second order accurate, the Suzuki weight being written ``1 / (4 * 4**(1/3))`` rather than ``1 / (4 - 4**(1/3))``. It now converges at fourth order, which for a Heisenberg chain at ``dt=0.05`` is about a thousand times more accurate. Note the correct weights include a negative one, so a fourth order step now evolves backwards over part of the step. The step schedule comes from [`trotter_schedule`](#trotter_schedule), and ``order=1`` is now accepted too.
- [`build_mpo_propagator_trotterized`](#LocalHam1D.build_mpo_propagator_trotterized): for a cyclic chain the wrapping term was applied with its two sites the wrong way round, giving an error linear rather than quadratic in ``x``. Terms symmetric under swapping the two sites, such as Heisenberg, were unaffected.
