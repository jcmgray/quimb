...
- [`tensor_split`](quimb.tensor.tensor_core.tensor_split): add `method="svd:rand"`, `"qr:rand"`, `"lq:rand"`, `"rfactor:rand"`, `"lfactor:rand"`, `"rorthog:rand"`, and `"lorthog:rand"` for randomized decompositions with static truncation, all via [`svd_rand_truncated`](quimb.tensor.decomp.svd_rand_truncated).
