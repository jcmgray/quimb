...
def get_smudge_balance(T1, T2, ix, smudge):
    ...
    if use_ad:
        ...
        for iblk2 in t2:
            ...
            if q0 not in block_dict:
                continue
            block_dict[q0] = block_dict[q0] / (np.diag(iblk2.data) + smudge)

        s1 = [
            block_cls(data=np.diag(s**-0.25), q_labels=(qlab,) * 2)
            for qlab, s in block_dict.items()
        ]
        s2 = [
            block_cls(data=np.diag(s**0.25), q_labels=(qlab,) * 2)
            for qlab, s in block_dict.items()
        ...
        for iblk2 in t2:
            ...
            if q0 not in block_dict:
                continue
            block_dict[q0] = block_dict[q0] / (
                np.diag(np.asarray(iblk2)) + smudge
            )

        s1 = [
            block_cls(reduced=np.diag(s**-0.25), q_labels=(qlab,) * 2)
            for qlab, s in block_dict.items()
        ]
        s2 = [
            block_cls(reduced=np.diag(s**0.25), q_labels=(qlab,) * 2)
