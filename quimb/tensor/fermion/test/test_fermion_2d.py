...
def u11setup(request):
    ...
    request.cls.peps = FPEPS.gen_site_prod_state(
        Lx, Ly, phys_infos, phys_dim=1
    )
...
class TestPEPS_U11:
    @pytest.mark.parametrize(
        "where",
        [(0, 0), (0, 1), (0, 2), (2, 0), (1, 0), (1, 1), (1, 2), (2, 1)],
    )
    ...
    @pytest.mark.parametrize("where", [[(1, 1), (2, 1)], [(2, 1), (2, 2)]])
    ...
    def test_compute_row_envs(self, two_layer):
        ...
        for i in range(norm.Lx):
            norm_i = (
                row_envs["bottom", i] & row_envs["mid", i] & row_envs["top", i]
            )
    ...
    def test_compute_col_envs(self, two_layer):
        ...
        for i in range(norm.Ly):
            norm_i = (
                row_envs["left", i] & row_envs["mid", i] & row_envs["right", i]
            )
            x = norm_i.contract(all)
            assert x == pytest.approx(ex, rel=1e-2)
    ...
    def test_compute_local_expectation_one_sites(self):
        ...
        expecs = peps.compute_local_expectation(
            terms, normalized=True, return_all=True
        )
    ...
    def test_compute_local_expectation_two_sites(self):
        ...
        opts = dict(
            cutoff=2e-3, max_bond=12, contract_optimize="random-greedy"
        )
