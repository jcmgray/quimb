import pytest
import numpy as np
from quimb.tensor.fermion.tensor_block import BlockTensor, BlockTensorNetwork
...
def u11setup(request):
    ...
    request.cls.Tabc = Tabc = BlockTensor(
        abc, inds=["a", "b", "c"], tags=["abc"]
    )
    request.cls.Tega = Tega = BlockTensor(
        ega, inds=["e", "g", "a"], tags=["ega"]
    )
    request.cls.Tbcd = Tbcd = BlockTensor(
        bcd, inds=["b", "c", "d"], tags=["bcd"]
    )
    request.cls.Tdeg = Tdeg = BlockTensor(
