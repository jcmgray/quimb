...
class TestNumpyEigk:
    ...
    def test_evals(self, ham1, which, k, ls, sigma):
        lk = eigs_numpy(
            ham1, k=k, which=which, return_vecs=False, sigma=sigma, sort=False
        )
        assert_allclose(lk, ls)

    @mark.parametrize("which, k, sigma", [("sa", 5, None)])
    def test_evecs(self, ham1, which, k, sigma):
        lk, vk = eigs_numpy(
            ham1, k=k, which=which, return_vecs=True, sigma=sigma, sort=False
        )
