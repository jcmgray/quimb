...
#         xf = approx_spectral_function(A, fn, tol=0.1)
...
#         xf = approx_spectral_function(rho_ab, lambda x: x, tol=0.1)
...
#         xf = approx_spectral_function(rho_ab_pt, lambda x: x, R=10, K=6)
...
#         psi0 = dmrg.state.to_dense()
#         lne = logneg_subsys(psi0, [2] * n, sysa=sysa, sysb=sysb)
#         lnx = log2(approx_spectral_function(rho_ab_pt, lambda x: abs(x), R=10))
#         assert_allclose(lne, lnx, rtol=0.5, atol=0.1)
